...
# the gym we want to import is the *package* gym, and not the module gym
import gym
...
sys.path.insert(0,parentdir + '/create_trees')
...
from create_trees.featureset import Featureset
from create_trees.ruleset import Ruleset
...
# START ADDITION
#################
from create_trees.decision_tree import Decision_Tree
import pickle
import wandb
"""
ARGUMENTS:
...
# path to gym
if on_cluster:
    PTH_TO_GYM = '' ############## enter path to the gym folder
    TREE_FILE_NAME = params["treefile_name"] + '_treefile'
    ruleset_file = '../create_trees/rulesets/' + params["ruleset_name"] + '_ruleset.txt'
    featureset_file = '../create_trees/featuresets/' + \
                      params["featureset_name"] + '_featureset.txt'
    TREEPATH = '../create_trees/trees/' + TREE_FILE_NAME
else:
    PTH_TO_GYM = '' ############## enter path to the gym folder
    TREE_FILE_NAME = params["treefile_name"] + '_treefile'
    ruleset_file = '../create_trees/rulesets/' + \
                   params["ruleset_name"] + '_ruleset.txt'
    featureset_file = '../create_trees/featuresets/' + \
                      params["featureset_name"] + '_featureset.txt'
    TREEPATH = '../create_trees/trees/' + TREE_FILE_NAME
...
# path to saved trees
TREE = pickle.load(open(TREEPATH, 'rb'))
...
decision_tree = Decision_Tree.from_tree(TREE, featureset)
params["decision_tree"] = decision_tree

# create runs[i]
if not os.path.isdir(PTH_TO_GYM + '/runs/runs' + str(run_number) + "/"):
    os.mkdir(PTH_TO_GYM + '/runs/runs' + str(run_number) + "/")
...
class SimulatedNetworkEnv(gym.Env):
    def __init__(self,
        ...
        # START ADDITION
        #####################
        # init all cntrs for dictionary
        
        self.cntrs = {
            "cntr_bad" : 0,
            "cntr_bad_good_action" : 0,
            "cntr_bad_bad_action" : 0,
            "good_to_total_bad_state_ratio" : 0,
            "bad_to_total_bad_state_ratio" : 0,
            "good_to_bad_action_ratio" : 0,

            "cntr_undesirable_decision_tree" : 0,
            "cntr_desirable_decision_tree" : 0,
            "cntr_undesirable_absolute" : 0,
            "cntr_desirable_absolute" : 0
        }
        #####################
        # END ADDITION
        #####################
        ...
        self.reward_sum_after_change = 0.0
        self.reward_ewma_after_change = 0.0
    ...
    def step(self, actions):
        ...
        #####################
        # get sample - state
        sample = np.round((sender_obs[2::3]), 2)
        ...
        #     if sum(1 for _ in open('samples_to_train_from.txt')) < 200000:
        #         raw_sample = sender_obs[2::3]
        #         # log samples
        #         with open("samples_to_train_from.txt", "a+") as f:
        #             np.savetxt(f, raw_sample, fmt="%.2f", newline="  ")
        #             f.write("\n")

        # if we should apply the reward_modifier
        do_stuff_with_alpha = params["train"]
        if params["alpha_downscaling"] == 1.0:
            do_stuff_with_alpha = False

        if do_stuff_with_alpha:
            # reward modifier
            alpha = self.change_cntrs_and_reward(sample, action[0], reward)
            # change cntrs and get alpha
        ...
        self.reward_sum_after_change += reward
        self.reward_sum += orig_reward
    ...
    def reset(self):
        ...
        self.reward_ewma_after_change *= 0.99
        self.reward_ewma_after_change += 0.01*self.reward_sum_after_change
        print("Reward: %0.2f, Ewma Reward: %0.2f" % (self.reward_sum, self.reward_ewma))

        ######################
        # START ADDITION
        ######################
        # write the ewma value
        
        
        if self.cntrs["cntr_bad_bad_action"] != 0:
            self.cntrs["good_to_bad_action_ratio"] = self.cntrs["cntr_bad_good_action"] / self.cntrs["cntr_bad_bad_action"]
        if self.cntrs["cntr_bad"] != 0:
            self.cntrs["good_to_total_bad_state_ratio"] = self.cntrs["cntr_bad_good_action"] / self.cntrs["cntr_bad"]
            self.cntrs["bad_to_total_bad_state_ratio"] = self.cntrs["cntr_bad_bad_action"] / self.cntrs["cntr_bad"]

        if (self.cntrs["cntr_undesirable_absolute"] + self.cntrs["cntr_desirable_absolute"]) != 0:
            undes_to_all_absolute = self.cntrs["cntr_undesirable_absolute"] / (self.cntrs["cntr_undesirable_absolute"] + self.cntrs["cntr_desirable_absolute"])
        else:
            undes_to_all_absolute = 0

        if (self.cntrs["cntr_undesirable_decision_tree"] + self.cntrs["cntr_desirable_decision_tree"]) != 0:
            undes_to_all_decision_tree = self.cntrs["cntr_undesirable_decision_tree"] / (self.cntrs["cntr_undesirable_decision_tree"] + self.cntrs["cntr_desirable_decision_tree"])
        else:
            undes_to_all_decision_tree = 0


        if self.cntrs["cntr_desirable_absolute"] != 0:
            undes_to_des_absolute = self.cntrs["cntr_undesirable_absolute"] / (self.cntrs["cntr_desirable_absolute"])
        else:
            undes_to_des_absolute = 0
            
        if self.cntrs["cntr_desirable_decision_tree"] != 0:
            undes_to_des_decision_tree = self.cntrs["cntr_undesirable_decision_tree"] / (self.cntrs["cntr_desirable_decision_tree"])
        else:
            undes_to_des_decision_tree = 0

        # log the dictionary
        logdict = {
            'Ewma' : self.reward_ewma,
            'Ewma after change' : self.reward_ewma_after_change,
            'Bad State Freq' : self.cntrs["cntr_bad"],
            'Good Action Bad State Freq': self.cntrs["cntr_bad_good_action"],
            'Bad Action Bad State Freq': self.cntrs["cntr_bad_bad_action"],
            'Good To Total Bad State ratio': self.cntrs["good_to_total_bad_state_ratio"],
            'Bad To Total Bad State ratio': self.cntrs["bad_to_total_bad_state_ratio"],
            'Good To Bad Action Ratio': self.cntrs["good_to_bad_action_ratio"],
            'Undesirable Cntr - Decision Tree' : self.cntrs["cntr_undesirable_decision_tree"],
            'Desirable Cntr - Decision Tree' : self.cntrs["cntr_desirable_decision_tree"],
            'Undesirable Cntr - Absolute' : self.cntrs["cntr_undesirable_absolute"],
            'Desirable Cntr - Absolute' : self.cntrs["cntr_desirable_absolute"],
            'Undesirable to Total Ratio - Decision Tree' : undes_to_all_decision_tree,
            'Undesirable to Desirable Ratio - Decision Tree' : undes_to_des_decision_tree,
            'Undesirable to Total Ratio - Absolute' : undes_to_all_absolute,
            'Undesirable to Desirable Ratio - Absolute' : undes_to_des_absolute
        }
        wandb.log(logdict)

        # init all cntrs for dictionary

        for cntr in self.cntrs.keys():
            self.cntrs[cntr] = 0
    ...
    def change_cntrs_and_reward(self, sample, action, reward):
        """
        Change the reward and increment relevant counters
        :param sample: the state
        :param action: 1 if '>0' and -1 if '<0' and 0 if (=0)
        :param reward: the reward
        """
        alpha = 1

        # ignore res rule and check for the rest
        # if true -> the state is bad
        #badstate = params["ruleset"].check(sample)
        POS_LAB = 0
        # badstate = params["decision_tree"].classify(sample) == POS_LAB
        #
        #
        # # if the state is bad
        # if badstate:
        #     self.cntrs["cntr_bad"] += 1
        #     # now we want to compare the action we took with the action we wanted to take
        #     if action == 0:
        #         curr_action = 0
        #     elif action > 0:
        #         curr_action = 1
        #     elif action < 0:
        #         curr_action = -1
        #     orig_action = params["ruleset"].get_res_rule()
        #     if orig_action == curr_action:
        #         # the action stayed the same - bad
        #         self.cntrs["cntr_bad_bad_action += 1"]
        #         alpha = params["alpha_downscaling"] # * params["decision_tree"].calc_alpha(sample)
        #         if reward < 0 and alpha != 0:
        #             if alpha < 0:
        #                 alpha = -alpha
        #             else:
        #                 alpha = 1 / alpha
        #     elif orig_action != None:
        #         # the action changed
        #         self.cntrs["cntr_bad_good_action"] += 1

        # get undesirability from tree
        undesirable = (params["decision_tree"].classify(sample, res=(True, action)) == POS_LAB)

        if undesirable:
            # add to undesirable behavior
            self.cntrs["cntr_undesirable_decision_tree"] += 1
            alpha = params["alpha_downscaling"]
            if reward < 0 and alpha != 0:
                if alpha < 0:
                    alpha = -alpha
                else:
            ...
            self.cntrs["cntr_desirable_decision_tree"] += 1
            # do not modify rewards
            alpha = 1


        #####
        # Check if state-action pair is truly undesirable
        ch = params["ruleset"].check(sample, action)
        if ch:
            self.cntrs["cntr_undesirable_absolute"] += 1
        else:
            self.cntrs["cntr_desirable_absolute"] += 1
        #####
