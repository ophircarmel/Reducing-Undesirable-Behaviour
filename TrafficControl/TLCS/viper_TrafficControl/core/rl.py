...
def test_policy(student, n_test_rollouts, it):
    ...
    for i in range(n_test_rollouts):
        student_trace = student.get_rollout_student(it*100 + i + 500)
        cum_rew += sum((rew for _, _, rew in student_trace if rew < 0))
...
def train_dagger(teacher, student, max_iters, n_batch_rollouts, max_samples, train_frac,
    ...
    ### START ADDITION
    #####################
    # try loading created pickled
    try:
        with open('obss_new.pkl', 'rb') as f:
            obss = pickle.load(f)
        with open('acts_new.pkl', 'rb') as f:
            acts = pickle.load(f)
        with open('qs_new.pkl', 'rb') as f:
            qs = pickle.load(f)
    #####################
    ### END ADDITION
    #####################
        ...
        cur_rew = sum((rew for _, _, rew in trace if rew < 0)) / n_batch_rollouts
    ...
    #####################
        # pickling
        with open('obss_new.pkl', 'wb+') as f:
            pickle.dump(obss, f)

        with open('acts_new.pkl', 'wb+') as f:
            pickle.dump(acts, f)

        with open('qs_new.pkl', 'wb+') as f:
            pickle.dump(qs, f)

    try:
        with open('students_new.pkl', 'rb') as f:
    ...
    for i in range(len(students), max_iters):
        ...
        cur_rew = sum((rew for _, _, rew in student_trace if rew < 0)) / n_batch_rollouts
        ...
        # pickling
        try:
            with open('students_new.pkl', 'wb+') as f:
