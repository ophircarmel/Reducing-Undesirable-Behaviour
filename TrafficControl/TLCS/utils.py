...
def import_train_configuration(config_file):
    ...
    config['reward_modifier'] = content['simulation'].getfloat('reward_modifier')
...
def import_test_configuration(config_file):
    ...
    config['green_duration'] = content['simulation'].getint('green_duration')
    config['yellow_duration'] = content['simulation'].getint('yellow_duration')
    config['init_seed'] = content['simulation'].getint('init_seed')
    config['num_states'] = content['agent'].getint('num_states')
    config['num_actions'] = content['agent'].getint('num_actions')
    config['sumocfg_file_name'] = content['dir']['sumocfg_file_name']
    config['models_path_name'] = content['dir']['models_path_name']
...
def set_specific_train_path(models_path_name, run_number):
    """
    Create a new model path with a specific integer
    """
    models_path = os.path.join(os   .getcwd(), models_path_name, '')
    os.makedirs(os.path.dirname(models_path), exist_ok=True)

    data_path = os.path.join(models_path, 'model_' + run_number, '')
    os.makedirs(os.path.dirname(data_path), exist_ok=True)
    return data_path
